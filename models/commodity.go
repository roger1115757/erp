...
func GetCommodities() (commodities []Commodity, err error) {
	err = db.Table("commodities").Select(
		"id, name, size, brand, number, presale_number, sales_volume, price").Find(&commodities).Error
...
func GetCommodity(id string) *Commodity {
	...
	db.Table("commodities").Where("id = ?", id).Select(
		"id, name, size, brand, number, presale_number, sales_volume, price").First(&commodity)
		// "id, name, colour, size, brand, number, presale_number, sales_volume, price").First(&commodity)
	return &commodity
}

func getPresaleNumber(tx *gorm.DB, goodsID string) (number int, err error) {
	info := struct {
		PresaleNumber int
	}{}
...
func updatePresaleNumber(tx *gorm.DB, goodsID string, number int) error {
	return tx.Table("commodities").Where("id = ?", goodsID).Update("presale_number", number).Error
}

func getNumber(tx *gorm.DB, goodsID string) (number int, err error) {
	info := struct {
		Number int
	}{}
...
func updateNumber(tx *gorm.DB, goodsID string, number int) error {
	fmt.Println(number)
	return tx.Table("commodities").Where("id = ?", goodsID).Update("number", number).Error
}

func getSalesVolume(tx *gorm.DB, goodsID string) (number int, err error) {
	info := struct {
		SalesVolume int
	}{}
