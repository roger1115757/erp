...
type PurchaseOrder struct {
	...
	Operate string `gorm:"size:255"`
	Remarks  string
	Amount   float32
	Freight  float32
	State    string `gorm:"default:'已完成'"`
	Goods []Commodity `gorm:"many2many:purchase_goods;"`
}

// PurchaseGoods 采购订单详细产品表
type PurchaseGoods struct {
	PurchaseOrderID uint `gorm:"primaryKey"`
	GoodsID         string `gorm:"primaryKey"`
	Number          int
}
...
func ListPurchaseOrders() (orders []PurchaseOrderInfo, err error) {
	db.SetupJoinTable(&PurchaseOrder{}, "Goods", &PurchaseGoods{})
	err = db.Table("purchase_orders").Select(
		"id, created_at, operator, amount, remarks, state, freight, operate").Find(&orders).Error
	if err != nil {
		return
	}
	...
	for index, v := range orders {
		...
		err = db.Raw(`select t2.id, t2.name, t2.size, t2.brand, t1.number from purchase_goods as t1 left outer join 
...
func getPurchaseOrder(tx *gorm.DB, orderID string) (*PurchaseOrderInfo, error) {
	...
	err = tx.Raw(`select t2.id, t2.name, t2.size, t2.brand, t1.number from purchase_goods as t1 left outer join
