...
func ListCustormerOrders() (orders []CustormerOrderInfo, err error) {
	...
	for index, v := range orders {
		...
		err = db.Raw(`select t2.id, t2.name, t2.size, t2.brand, t1.number  from custormer_goods as t1 left outer join 
...
func getCustormerOrder(tx *gorm.DB, orderID string) (*CustormerOrderInfo, error) {
	...
	err = tx.Raw(`select t2.id, t2.name, t2.size, t2.brand, t1.number  from custormer_goods as t1 left outer join 
